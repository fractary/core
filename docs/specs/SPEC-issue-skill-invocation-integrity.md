...
- **`--repo` hard-require rule**: When `--repo` is explicitly provided, it MUST be passed to the CLI. The `## Context` Repository field shows only the calling project and must not be used as the default when `--repo` is provided.
