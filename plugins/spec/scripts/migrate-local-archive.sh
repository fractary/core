...
# previously archived files in .fractary/specs/archive/ need to be uploaded
# to cloud storage. This script finds all locally archived spec/doc files,
...
LOCAL_ARCHIVE_DIR=".fractary/specs/archive"
...
echo "Found $FILE_COUNT locally archived spec/doc file(s) to migrate to cloud storage" >&2
...
while IFS= read -r FILE_PATH; do
    ...
    # e.g., .fractary/specs/archive/WORK-00123-auth.md -> WORK-00123-auth.md
    REL_PATH="${FILE_PATH#$LOCAL_ARCHIVE_DIR/}"

    # Cloud archive path mirrors the same structure
    CLOUD_PATH="archive/specs/${REL_PATH}"
    ...
    if ! UPLOAD_RESULT=$(node "$STORAGE_SCRIPT" upload specs "$FILE_PATH" "$CLOUD_PATH" 2>&1); then
